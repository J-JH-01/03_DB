...
SELECT INSTR('AABAACAABBAA','B', 5) FROM DUAL;  --8
...
-- 주어진 컬럼이나 문자열의 앞,뒤,양쪽에 잇는 지정된 문자 제거
--> 양쪽 공백 제거에 많이 사용함
...
-- EMPLOYEE 테이블에서 사원의 웕브을 100만으로 나눴을 때 나머지 조회
...
SELECT ROUND(123.456, 1) FROM DUAL; -- 123, 소수점 두번째 자리에서 반올림(소수점 첫번째 자리까지 표기)
...
-- MONTHS.BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이 반환
...
-- HH : 시간 : HH24 : 24시간 표기법
-- MI : 분 // SS : 초
-- DAY : 요일(전체) / DY : 요일(요일명만 표시,월,금 이렇게)
