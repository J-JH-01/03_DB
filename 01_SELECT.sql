...
-- '*' : ALL ,모든
--> EMPLOYEE 테이블의 모든 칼럼을 조회하겠다

-- EMPLOYEE 테이블에서 사번, 직원이름, 휴대전화번호 칼럼만 조회
...
-- EMPLOYEE 테이블에서 이름, 입사일, 오늘 날짜 조회(SYSDATE)
SELECT EMP_NAME, HIRE_DATE , SYSDATE FROM EMPLOYEE;

-- 2024-10-08 11:10:10.000
-- SYSDATE : 시스템상의 현재 시간(날짜)를 나타내는 상수

SELECT SYSDATE FROM DUAL;
--DUAL(DUmmy tAbLe -> DUAL) 테이블 : 가짜 테이블(임시 조회용 테이블)
...
-- 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC](오름차순{디폴트},내림차순) [NULLS FIRST | LAST]

-- 별칭을 SELECT절에서 넣었다하면 2번 절에선 못쓰지만 4번절에선 사용이 가능

-- EMPLOYEE 테이블에서 (FROM 절)
--급여가 300만원 초과인 사원의 (WHERE 절)
...
-- ** AND가 OR보다 우선순위가 높기 때문에 OR을 섞어쓸땐 ()로 순위를 높여줘야한다

SELECT EMP_ID ,EMP_NAME ,EMAIL ,DEPT_CODE ,SALARY 
FROM EMPLOYEE
...
ORDER BY 3 DESC ; --SELECT의 3번째 컬럼 기준으로 정렬하겠다


-- 입사일 순서대로 이름,입사일 조회(별칭 사용)
-- SELECT가 ORDER BY보다 순서가 빨라서 가능
SELECT EMP_NAME 이름,HIRE_DATE "입사일"
FROM EMPLOYEE
ORDER BY 입사일;

/* 정렬 중첩 : 대분류 정렬 후 소분류 정렬
 -- 부서 코드 오름 차순 정렬 후 급여 내림 차순 정렬
 * */
